...
from zoneinfo import ZoneInfo  # <-- NEW: pour gérer le DST correctement
...
def tokyo_signal_window(d: date) -> Tuple[int, int]:
    # Inchangé : fenêtre UTC fixe
    base = datetime(d.year, d.month, d.day, tzinfo=UTC)
    debut = int((base + timedelta(hours=1)).timestamp()*1000)               # 01:00 UTC
    fin   = int((base + timedelta(hours=5, minutes=45)).timestamp()*1000)   # 05:45 UTC
    return debut, fin

# Helper générique pour fenêtre locale -> UTC, durée = 4h45 (soit 5h de bougies 15m de 00 à :45 inclus)
def _session_window_local(d: date, tzname: str, start_hour: int, start_minute: int) -> Tuple[int, int]:
    # construit l'heure de début en heure locale de la place (DST-aware), puis convertit en UTC
    start_local = datetime(d.year, d.month, d.day, start_hour, start_minute, tzinfo=ZoneInfo(tzname))
    end_local   = start_local + timedelta(hours=4, minutes=45)  # dernière bougie à :45
...
def london_signal_window(d: date) -> Tuple[int, int]:
    # 08:00 Europe/London local, fenêtre 5h (dernière bougie 12:45 local) — DST inclus
    return _session_window_local(d, "Europe/London", 8, 0)

def ny_signal_window(d: date) -> Tuple[int, int]:
    # 13:00 America/New_York local, fenêtre 5h (dernière bougie 17:45 local) — DST inclus
    return _session_window_local(d, "America/New_York", 13, 0)
