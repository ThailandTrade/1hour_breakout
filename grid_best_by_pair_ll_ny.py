...
def day_ms_bounds(d: date) -> Tuple[int, int]:
    start = datetime(d.year, d.month, d.day, 13, 00, tzinfo=UTC)
...
def ny_signal_window(d: date) -> Tuple[int, int]:
    base = datetime(d.year, d.month, d.day, tzinfo=UTC)
    start = int((base + timedelta(hours=14)).timestamp()*1000)   # 13:00
    end   = int((base + timedelta(hours=18, minutes=45)).timestamp()*1000)   # 17:45
    return start, end
