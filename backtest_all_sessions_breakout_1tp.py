...
def evaluate_trade_tp_only(conn, pair: str, tr: Trade, tp_level: str):
    ...
    for b in future:
        ...
        # priorité : quel évènement survient en premier
...
def run_all(conn,
    ...
    for d in daterange(start, end):
        for sess, pair, tp_level in session_pairs:
            ...
            seen_sessions.add(key)  # on bloque toute reprise pour cette session/paire/jour
...
def load_session_file(path: str) -> List[Tuple[str, str, str]]:
    ...
    with open(path, "r", newline="") as f:
        for raw in f:
            ...
            pair = parts[1].upper()
            tp   = parts[2].upper()
            if tp not in ("TP1","TP3","TP2"):  # keep same validation but tolerant to order
                continue
...
def main():
    ...
    # Defaults wired to top-level parameters so you can tweak them there
