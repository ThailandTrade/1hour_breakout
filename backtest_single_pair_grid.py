...
# -*- coding: utf-8 -*-
"""
Single Pair — Session × (w1,w2,w3) Grid Search (step 0.1) — Expectancy Table

- Entrées: même logique (break strict, pullback antagoniste, entrée sur wick, SL = bar i-1).
- Cibles: RR1 / RR2 / RR3 (timestamps), arrêt au 1er SL ou RR3.
- WIN/LOSS: WIN si TP1 avant SL, sinon LOSS (indépendant des poids).
- R-multiple: application événementielle des partiels selon (w1,w2,w3).
- Sessions: TOKYO / LONDON / NY (fenêtres UTC paramétrées ci-dessous).
- Sortie: tableau trié par Expectancy (R) avec:
    Session | w1 | w2 | w3 | Trades | Winrate | AvgWinR | AvgLossR | ExpectancyR | TP1% | TP2% | TP3%

NB: pas de frais / sizing en $ dans cette version: on optimise la combinaison de R.
"""
...
def evaluate_trade_after_entry(conn, pair: str, tr: Trade):
    """
    Enregistre les timestamps de RR1/RR2/RR3/SL et s'arrête au premier SL ou RR3.
    """
...
def compute_r_and_close(hit_time: Dict[str, Optional[int]], w1: float, w2: float, w3: float) -> float:
    """
    Application temporelle des sorties partielles:
      TP1: +w1 * 1R ; rem -= w1
      TP2: +w2 * 2R ; rem -= w2
      TP3: +w3 * 3R ; rem -= w3
      SL : -1R * rem (sur la taille restante)
    Renvoie seulement le R-multiple total (pas besoin du close_ts pour l’optimisation).
    """
...
def collect_trades_for_session(conn, pair: str, start: date, end: date, session: str) -> List[BareTrade]:
    ...
    for d in daterange(start, end):
        ...
        # ⚠️ Empêche d'ouvrir un trade dont l'entrée est avant la clôture du précédent
...
def weight_grid(step: float = 0.1):
    ...
    for w1 in vals:
        for w2 in vals:
            ...
            if w3 < -1e-9: 
                continue
            # forcer à l’ensemble {0,0.1,...,1.0} avec somme = 1.0
...
def print_grid_table(pair: str, rows: List[Dict[str, Any]]):
    try:
    ...
    if PrettyTable:
        ...
        print("\n===== GRID RECAP — trié par Expectancy (R) =====")
        print(t)
        print("================================================")
    else:
        # fallback simple
        print("\n===== GRID RECAP — trié par Expectancy (R) =====")
        header = ["Pair","Session","TP1","TP2","TP3","Trades","Winrate","AvgWinR","AvgLossR","ExpectancyR","TP1%","TP2%","TP3%"]
        print("\t".join(header))
        for r in rows_sorted:
            print("\t".join([
                pair, r["session"],
...
def main():
    ...
    ap.add_argument("--pair", required=True, help="Paire à tester (ex: EURUSD)")
    ...
    with get_pg_conn() as c:
        # 1) on collecte les trades (hits) UNE FOIS par session
        session_trades: Dict[str, List[BareTrade]] = {}
        for sess in sessions:
            print(f"Collecte des trades pour {pair} — {sess} ...")
            session_trades[sess] = collect_trades_for_session(c, pair, d0, d1, sess)

        # 2) grille des poids et stats pour chaque session
        ...
        # 3) affichage trié par expectancy
        print_grid_table(pair, all_rows)
