...
TP_WEIGHTS = (0.0, 1.0, 0.0)  # must sum to 1.0 (e.g., 0.5,0.25,0.25 or 1,0,0)
# ------------------------------------------
...
def newyork_signal_window(d: date) -> Tuple[int, int]:
    base  = datetime(d.year, d.month, d.day, tzinfo=UTC)
    start = int((base + timedelta(hours=12)).timestamp()*1000)              # 12:00 UTC
    end   = int((base + timedelta(hours=16, minutes=45)).timestamp()*1000)  # 16:45 UTC
    return start, end
